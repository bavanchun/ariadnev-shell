...
in
{
  ...
  options.programs.ariadnev-shell.lockscreen.securityKey = {
    ...
    moduleArgs = lib.mkOption {
      ...
      default = [ "cue" ];
      description = "Arguments passed to pam_u2f.so in the advshell-u2f PAM service.";
    };
  };
  config = lib.mkIf cfg.enable {
    ...
    environment.etc = lib.mapAttrs' (name: value: {
      name = "xdg/quickshell/ariadnev-plugins/${name}";
      inherit value;
    }) common.plugins;

    # ADVS's bundled U2F fallback stack references pam_u2f.so by name, which NixOS's
    ...
    security.pam.services."advshell-u2f" = lib.mkIf cfg.lockscreen.securityKey.enable {
      text = ''
