---
name: advs-plugin-dev
description: >
  Develop plugins for AriadnevShell (ADVS), a QML-based Linux desktop shell built on
  ...
  or debug a ADVS plugin, or asks about the ADVS plugin API.
compatibility: Designed for Claude Code (or similar products)
metadata:
  author: AriadnevShell
  version: "1.1"
  domain: qml-desktop-development
  framework: AriadnevShell
  languages: qml, javascript
allowed-tools: Bash Read Write Edit
---

# AriadnevShell Plugin Development

## Overview

ADVS plugins extend the desktop shell with custom widgets, background services, launcher
integrations, and desktop widgets. Plugins are QML components discovered from
`~/.config/AriadnevShell/plugins/`.

**Minimum plugin structure:**

```
~/.config/AriadnevShell/plugins/YourPlugin/
...
- **Shows in the bar?** - Use `widget`. Displays a pill in AdvBar, optionally opens a popout,
...
- **Provides searchable/actionable items?** - Use `launcher`. Items appear in the ADVS launcher
...
Item {
    ...
    function executeItem(item) {
        ...
        else if (type === "copy") Quickshell.execDetached(["advs", "cl", "copy", data])
    }
}
```
...
**Common widgets from `qs.Widgets`:** `StyledText`, `StyledRect`, `AdvIcon`, `AdvButton`, `AdvToggle`, `AdvTextField`, `AdvSlider`, `AdvGridView`, `CachingImage`.
...
Quickshell.execDetached(["advs", "cl", "copy", textToCopy])
```
...
Ship per-locale files in a `translations/` directory; ADVS loads the one matching the active locale and re-reads it on locale changes:
...
File names follow ADVS locale naming (`es.json`, `pt.json`, `zh_CN.json`); resolution tries the exact tag, then the hyphenated form, then the bare language. No `en.json` - the QML terms are the English strings.
...
Lookup order: plugin file, then the global ADVS catalog, then the English term. `ExampleEmojiPlugin` in `quickshell/PLUGINS/` demonstrates the full setup. Registry plugins can apply for central POEditor translation via the registry's CONTRIBUTING guide.
...
2. Run the shell with verbose output: `qs -v -p $CONFIGPATH/quickshell/ariadnev/shell.qml`
3. Open Settings > Plugins > Scan for Plugins
4. Enable your plugin and add it to the AdvBar layout
...
```bash
advs ipc plugin-scan scan          # Trigger a full rescan of all plugin directories
advs ipc plugin-scan rescan <id>   # Force rescan of a specific plugin
advs ipc plugin-scan reload <id>   # Force reload of a loaded plugin
advs ipc plugin-scan list          # List all known plugins (TSV: id, loaded, type, name)
advs ipc plugin-scan status <id>   # Get status of a specific plugin (TSV: loaded, type, error)
```
...
- Widget not showing: ensure it's enabled AND added to a AdvBar section
...
- Startup check failing: check `pluginService.pluginLoadErrors` or run `advs ipc plugin-scan status <id>`
...
5. **Using `globalThis.clipboard`** - Does not exist; use `Quickshell.execDetached(["advs", "cl", "copy", text])`
