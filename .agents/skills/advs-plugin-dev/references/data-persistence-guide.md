...
ADVS plugins have three tiers of data persistence, each suited for different use cases.
...
Returns the absolute path to the plugin's directory (e.g., `~/.config/AriadnevShell/plugins/MyPlugin`), or an empty string if the plugin is not found. Useful for loading bundled assets (images, data files) relative to the plugin's location.
