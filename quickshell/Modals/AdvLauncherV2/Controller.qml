...
Item {
    ...
    Connections {
        ...
        function onLauncherSearchReady(query) {
            ...
            const clipboardBuiltInActive = activePluginId === "advs_clipboard_search";
            ...
            const effectiveQuery = clipboardBuiltInActive && triggerMatch.pluginId === "advs_clipboard_search" ? triggerMatch.query : trimmed;
    ...
    function builtInSectionViewPref(sectionId) {
        switch (sectionId) {
        case "clipboard":
            return getPluginViewPref("advs_clipboard_search");
        case "settings":
            return getPluginViewPref("advs_settings_search");
        default:
            return null;
        }
    }
    ...
    function setSearchQuery(query) {
        ...
        var filesInAll = searchMode === "all" && (SettingsData.advLauncherV2IncludeFilesInAll || SettingsData.advLauncherV2IncludeFoldersInAll);
    ...
    function setMode(mode, isAutoSwitch, fileTypeOverride, notPersist) {
        ...
        performSearch();
        var filesInAll = mode === "all" && (SettingsData.advLauncherV2IncludeFilesInAll || SettingsData.advLauncherV2IncludeFoldersInAll) && searchQuery.length > 0;
        ...
        if (searchMode === "apps") {
            ...
            if (isCategoryFiltered) {
                ...
                if (categoryPluginId) {
                    ...
                    // Also include core apps (ADVS Settings etc.) that match this category
    ...
    function _performPluginPhase() {
        ...
        if (!searchQuery) {
            ...
            for (var i = 0; i < allPluginsOrdered.length; i++) {
                ...
                if (plugin.isBuiltIn && (plugin.id === "advs_settings_search" || plugin.id === "advs_clipboard_search"))
                    continue;
                if (plugin.isBuiltIn) {
                    var blItems = AppSearchService.getBuiltInLauncherItems(plugin.id, searchQuery);
    ...
    function performFileSearch() {
        ...
        var includeFiles = SettingsData.advLauncherV2IncludeFilesInAll;
        var includeFolders = SettingsData.advLauncherV2IncludeFoldersInAll;
    ...
    function transformBuiltInSearchItem(item, pluginId) {
        if (pluginId === "advs_clipboard_search" || item.type === "clipboard") {
    ...
    function clipboardSearchEnabledInAll() {
        return builtInLauncherVisibleInAll("advs_clipboard_search") && ClipboardService.clipboardAvailable;
    }
    ...
    function appendSharedAllResults(allItems, query) {
        ...
        if (builtInLauncherVisibleInAll("advs_settings_search")) {
            var settingsItems = AppSearchService.getBuiltInLauncherItems("advs_settings_search", query);
            var settingsLimit = Math.min(settingsItems.length, 8);
            for (var i = 0; i < settingsLimit; i++) {
                settingsItems[i]._preScored = 890 - i;
                allItems.push(transformBuiltInSearchItem(settingsItems[i], "advs_settings_search"));
            }
        }

        if (clipboardSearchEnabledInAll()) {
            var clipboardItems = AppSearchService.getBuiltInLauncherItems("advs_clipboard_search", query);
            var clipboardLimit = Math.min(clipboardItems.length, 8);
            for (var j = 0; j < clipboardLimit; j++) {
                clipboardItems[j]._preScored = 840 - j;
                allItems.push(transformBuiltInSearchItem(clipboardItems[j], "advs_clipboard_search"));
            }
        }
    }
    ...
    function buildDynamicSectionDefs(items) {
        ...
        for (var d = 0; d < baseDefs.length; d++) {
            var virtualId = baseDefs[d].id === "settings" ? "advs_settings_search" : baseDefs[d].id === "clipboard" ? "advs_clipboard_search" : "";
