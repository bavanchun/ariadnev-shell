...
PanelWindow {
    id: root
    readonly property var log: Log.scoped("AdvOSD")

    property string blurNamespace: "advs:osd"
    ...
    WlrLayershell.layer: LayerShell.fromEnv("ADVS_OSD_LAYER", WlrLayer.Overlay, {
        "allow": ["top", "overlay"],
    ...
    readonly property var barEdgeOffsets: {
        ...
        offsets.top = Math.max(offsets.top, SettingsData.advIslandEdgeOffset(screen, "top"));
        offsets.bottom = Math.max(offsets.bottom, SettingsData.advIslandEdgeOffset(screen, "bottom"));
        return offsets;
    }
    ...
    Item {
        ...
        ElevationShadow {
            ...
            borderWidth: 1
            shadowEnabled: Theme.elevationEnabled && SettingsData.popoutElevationEnabled && Quickshell.env("ADVS_DISABLE_LAYER") !== "true" && Quickshell.env("ADVS_DISABLE_LAYER") !== "1"
        }
