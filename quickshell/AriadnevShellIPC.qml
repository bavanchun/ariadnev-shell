...
Item {
    id: root
    readonly property var log: Log.scoped("AriadnevShellIPC")
    ...
    required property var advDashPopoutLoader
    required property var notepadSlideoutVariants
    required property var hyprKeybindsModalLoader
    required property var advBarRepeater
    ...
    function getPreferredBar(refPropertyName) {
        ...
        const bars = [];
        if (root.advBarRepeater) {
            for (let i = 0; i < root.advBarRepeater.count; i++)
                bars.push(...(root.advBarRepeater.itemAt(i)?.item?.barVariants?.instances || []));
        }
    ...
    IpcHandler {
        ...
        function _openDash(tab, position) {
            ...
            const dash = root.advDashPopoutLoader.item;
        ...
        function _toggleDash(tab, position) {
            if (root.advDashPopoutLoader.item?.dashVisible) {
                root.advDashPopoutLoader.item.dashVisible = false;
                return true;
            }
        ...
        function close(): string {
            ...
            if (root.advDashPopoutLoader.item) {
                root.advDashPopoutLoader.item.dashVisible = false;
                return "DASH_CLOSE_SUCCESS";
            }
    ...
    IpcHandler {
        function wallpaper(): string {
            if (PopoutService.routeToIsland("wallpaper", null, true))
                return "WARN; deprecated, use advs ipc call dash toggle wallpaper instead";
    ...
    IpcHandler {
        ...
        function openWith(mode: string): string {
            ...
            PopoutService.openAdvLauncherV2WithMode(mode);
