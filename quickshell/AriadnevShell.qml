...
import qs.Modules.AdvBar.Popouts
...
Item {
    id: root
    readonly property var log: Log.scoped("AriadnevShell")
    ...
    readonly property var advIslandScreens: SettingsData.getIslandScreens()
    readonly property var notificationPopupScreens: {
        ...
        return root.withoutAdvIslandScreens(screens);
    }
    readonly property var legacySystemLevelOsdScreens: root.withoutAdvIslandScreens(SettingsData.getFilteredScreens("osd"))

    function withoutAdvIslandScreens(screens) {
        if (!SettingsData.advIslandEnabled)
            return screens;
        return screens.filter(screen => root.advIslandScreens.indexOf(screen) === -1);
    }
    ...
    Loader {
        id: advDashPopoutLoader

        active: false
        asynchronous: false

        Component.onCompleted: {
            PopoutService.advDashPopoutLoader = advDashPopoutLoader;
        }

        onLoaded: {
            if (item) {
                PopoutService.advDashPopout = item;
                PopoutService._onAdvDashPopoutLoaded();
            }
        }
        ...
        sourceComponent: Component {
            AdvDashPopout {
                ...
                onPopoutClosed: PopoutService._scheduleUnload("advDash")
            }
        }
    }
    ...
    LazyLoader {
        id: advLauncherV2ModalLoader

        active: false

        Component.onCompleted: {
            PopoutService.advLauncherV2ModalLoader = advLauncherV2ModalLoader;
        }

        AdvLauncherV2Modal {
            id: advLauncherV2Modal

            Component.onCompleted: {
                PopoutService.advLauncherV2Modal = advLauncherV2Modal;
                PopoutService._onAdvLauncherV2ModalLoaded();
            }
        }
    }
    ...
    Connections {
        ...
        function onOpenUrlRequested(url) {
            if (url.startsWith("advs://theme/install/")) {
                var themeId = url.replace("advs://theme/install/", "").split(/[?#]/)[0];
                if (themeId) {
            ...
            if (url.startsWith("advs://plugin/install/")) {
                var pluginId = url.replace("advs://plugin/install/", "").split(/[?#]/)[0];
                if (pluginId) {
    ...
    AriadnevShellIPC {
        ...
        advDashPopoutLoader: advDashPopoutLoader
        notepadSlideoutVariants: notepadSlideoutVariants
        hyprKeybindsModalLoader: hyprKeybindsModalLoader
        advBarRepeater: root.core?.advBarRepeater ?? null
