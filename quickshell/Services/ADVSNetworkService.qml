...
Singleton {
    id: root
    readonly property var log: Log.scoped("ADVSNetworkService")
    ...
    readonly property bool hotspotSupported: ADVSService.isConnected && networkAvailable && ADVSService.apiVersion >= hotspotApiVersion && backendHotspotSupported
    ...
    function checkADVSCapabilities() {
        ...
        networkAvailable = ADVSService.capabilities.includes("network");
    ...
    function getState() {
        if (!networkAvailable)
            return;
        ADVSService.sendRequest("network.getState", null, response => {
            if (response.result) {
    ...
    function updateState(state) {
        ...
        if (ADVSService.apiVersion >= hotspotApiVersion) {
        ...
        if (state.wifiNetworks || state.savedWifiNetworks) {
            const hasSavedWifiState = ADVSService.apiVersion >= savedWifiStateApiVersion && Array.isArray(state.savedWifiNetworks);
    ...
    function connectToSpecificWiredConfig(uuid) {
        ...
        ADVSService.sendRequest("network.ethernet.connect.config", params, response => {
            if (response.error) {
    ...
    function connectCellular() {
        ...
        ADVSService.sendRequest("network.cellular.connect", null, response => {
            if (response.error) {
                connectionError = response.error;
                lastConnectionError = response.error;
                connectionStatus = "failed";
    ...
    function connectToSpecificCellularConfig(uuid) {
        ...
        ADVSService.sendRequest("network.cellular.connect.config", {
            uuid: uuid
        }, response => {
            if (response.error) {
                connectionError = response.error;
    ...
    function scanWifi() {
        ...
        ADVSService.sendRequest("network.wifi.scan", params, response => {
            isScanning = false;
            if (response.error) {
    ...
    function connectToWifi(ssid, password = "", username = "", anonymousIdentity = "", domainSuffixMatch = "", hidden = false, eapMethod = "", phase2Auth = "") {
        ...
        ADVSService.sendRequest("network.wifi.connect", params, response => {
            if (response.error) {
    ...
    function disconnectWifi() {
        ...
        ADVSService.sendRequest("network.wifi.disconnect", params, response => {
            if (response.error) {
    ...
    function submitCredentials(token, secrets, save) {
        log.debug("submitCredentials: networkAvailable=" + networkAvailable + " apiVersion=" + ADVSService.apiVersion);

        if (!networkAvailable || ADVSService.apiVersion < 7) {
            log.warn("submitCredentials: Aborting - networkAvailable=" + networkAvailable + " apiVersion=" + ADVSService.apiVersion);
            return;
        }
        ...
        ADVSService.sendRequest("network.credentials.submit", params, response => {
            if (response.error) {
    ...
    function cancelCredentials(token) {
        ...
        ADVSService.sendRequest("network.credentials.cancel", params, response => {
            if (response.error) {
    ...
    function forgetWifiNetwork(ssid) {
        if (!networkAvailable)
            return;
        forgetSSID = ssid;
        ADVSService.sendRequest("network.wifi.forget", {
            ssid: ssid
        }, response => {
            if (response.error) {
    ...
    function toggleWifiRadio() {
        if (!networkAvailable || wifiToggling)
            return;
        wifiToggling = true;
        ADVSService.sendRequest("network.wifi.toggle", null, response => {
            wifiToggling = false;
    ...
    function toggleCellularRadio() {
        ...
        ADVSService.sendRequest("network.cellular.toggle", null, response => {
    ...
    function enableWifiDevice() {
        if (!networkAvailable)
            return;
        ADVSService.sendRequest("network.wifi.enable", null, response => {
            if (response.error) {
    ...
    function setNetworkPreference(preference) {
        ...
        ADVSService.sendRequest("network.preference.set", {
    ...
    function toggleNetworkConnection(type) {
        ...
        if (type === "ethernet") {
            if (ethernetConnected) {
                ADVSService.sendRequest("network.ethernet.disconnect", null, null);
            } else {
                ADVSService.sendRequest("network.ethernet.connect", null, null);
            }
        } else if (type === "cellular") {
            if (cellularConnected) {
                ADVSService.sendRequest("network.cellular.disconnect", null, null);
            } else {
                connectCellular();
            }
        }
    ...
    function disconnectEthernetDevice(deviceName) {
        if (!networkAvailable)
            return;
        ADVSService.sendRequest("network.ethernet.disconnect", {
            device: deviceName
        }, null);
    }
    ...
    function disconnectCellularDevice(deviceName) {
        if (!networkAvailable)
            return;
        ADVSService.sendRequest("network.cellular.disconnect", {
            device: deviceName
        }, null);
    }
    ...
    function fetchWiredNetworkInfo(uuid) {
        ...
        ADVSService.sendRequest("network.ethernet.info", {
            uuid: uuid
        }, response => {
    ...
    function fetchNetworkInfo(ssid) {
        ...
        ADVSService.sendRequest("network.info", {
            ssid: ssid
        }, response => {
    ...
    function refreshVpnProfiles() {
        if (!vpnAvailable)
            return;
        ADVSService.sendRequest("network.vpn.profiles", null, response => {
            if (response.result) {
    ...
    function refreshVpnActive() {
        if (!vpnAvailable)
            return;
        ADVSService.sendRequest("network.vpn.active", null, response => {
            if (response.result) {
    ...
    function connectVpn(uuidOrName, singleActive = false) {
        ...
        ADVSService.sendRequest("network.vpn.connect", params, response => {
            if (response.error) {
                vpnIsBusy = false;
                pendingVpnUuid = "";
                vpnBusyStartTime = 0;
    ...
    function disconnectVpn(uuidOrName) {
        ...
        ADVSService.sendRequest("network.vpn.disconnect", params, response => {
            if (response.error) {
                vpnIsBusy = false;
                vpnBusyStartTime = 0;
    ...
    function disconnectAllVpns() {
        ...
        ADVSService.sendRequest("network.vpn.disconnectAll", null, response => {
            if (response.error) {
                vpnIsBusy = false;
    ...
    function setWifiAutoconnect(ssid, autoconnect) {
        if (!networkAvailable || ADVSService.apiVersion <= 13)
            return;
        const params = {
            ssid: ssid,
        ...
        ADVSService.sendRequest("network.wifi.setAutoconnect", params, response => {
            if (response.error) {
    ...
    function configureHotspot(ssid, password = "", device = "", band = "", callback = null) {
        ...
        ADVSService.sendRequest("network.hotspot.configure", params, response => {
            hotspotBusy = false;
            if (response.error) {
    ...
    function startHotspot(callback = null) {
        ...
        ADVSService.sendRequest("network.hotspot.start", null, response => {
            hotspotBusy = false;
            if (response.error) {
                hotspotError = response.error;
    ...
    function stopHotspot(callback = null) {
        ...
        ADVSService.sendRequest("network.hotspot.stop", null, response => {
            hotspotBusy = false;
            if (response.error) {
                hotspotError = response.error;
    ...
    function getHotspotSecrets(callback) {
        ...
        ADVSService.sendRequest("network.hotspot.getSecrets", null, response => {
            if (callback)
                callback(response);
        });
