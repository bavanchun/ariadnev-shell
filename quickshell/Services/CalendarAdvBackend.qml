...
Item {
    id: root
    readonly property var log: Log.scoped("CalendarAdvBackend")
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (!root.connected) {
                    if (root.socketPath !== "")
                        root.log.info("advcal socket gone, waiting for daemon");
                    requestSocket.connected = false;
                    subscribeSocket.connected = false;
                    root.socketPath = "";
                }
    ...
    function _applySocketPath(path) {
        ...
        if (changed)
            log.info("advcal socket discovered:", path);
    ...
    AdvSocket {
        ...
        onConnectionStateChanged: {
            if (linkUp) {
                ...
                root.log.info("connected to advcal:", root.socketPath);
            ...
            requestSocket.connected = false;
            subscribeSocket.connected = false;
            root.log.info("advcal disconnected, rediscovering");
            if (root.enabled)
                discoverProcess.running = true;
        }
    ...
    function sendRequest(method, params, callback) {
        if (!connected) {
            if (callback)
                callback({
                    "error": "not connected to advcal socket"
                });
            return;
        }
        const id = _nextId();
