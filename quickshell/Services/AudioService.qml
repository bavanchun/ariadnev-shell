...
Singleton {
    ...
    property string wireplumberConfigPath: Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation)) + "/wireplumber/wireplumber.conf.d/51-advs-audio-aliases.conf"
...
`;
    ...
    function generateWireplumberConfig() {
        let config = "# Generated by AriadnevShell - Audio Device Aliases\n";
    ...
    function playLoginSoundIfApplicable() {
        if (SettingsData.soundsEnabled && SettingsData.soundLogin && !notificationsAudioMuted) {
            ...
            // to prevent it from playing on every ADVS restart during the session
            ...
            const loginFile = `${runtimeDir}/ariadnev.login-${sessionId}`;
