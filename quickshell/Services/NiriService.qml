...
Singleton {
    ...
    // advs/layout.kdl is the source of truth for xray; parsed once before the first regeneration
    ...
    function handleScreenshotCaptured(data) {
        ...
        if (pendingScreenshotPath && data.path === pendingScreenshotPath) {
            const editor = Quickshell.env("ADVS_SCREENSHOT_EDITOR");
            let command;
    ...
    function screenshot() {
        ...
        const path = `${screenshotsDir}/advs-screenshot-${timestamp}.png`;
    ...
    function loadLayoutXrayState() {
        ...
        Proc.runCommand("niri-read-layout-xray", ["cat", configDir + "/niri/advs/layout.kdl"], (output, exitCode) => {
    ...
    function doGenerateNiriLayoutConfig() {
        ...
        // advs:frame only in separate mode — connected-mode frame blur overlaps windows via popouts/arcs
        const excludeNamespaces = ["advs:bar"];
        if (typeof SettingsData !== "undefined" && SettingsData.advIslandEnabled)
            excludeNamespaces.push("advs:advisland");
        if (frameEnabled && SettingsData.frameMode !== "connected")
            excludeNamespaces.push("advs:frame");
...
window-rule {
    match app-id="^dev.vchun.ariadnev$"
    background-effect {
        xray false
    }
}`;
...
// bar-xray off`;
        ...
        const advsWarning = `// ! DO NOT EDIT !
// ! AUTO-GENERATED BY ADVS !
...
`;
        ...
        const configContent = advsWarning + `layout {
${layoutLines.join("\n")}
}
...
window-rule {
    ...
    draw-border-with-background false
        ...
        const alttabContent = advsWarning + `recent-windows {
    highlight {
        ...
        const configDir = Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation));
        const niriAdvsDir = configDir + "/niri/advs";
        const configPath = niriAdvsDir + "/layout.kdl";
        const alttabPath = niriAdvsDir + "/alttab.kdl";
        ...
        writeConfigProcess.command = ["sh", "-c", `mkdir -p "${niriAdvsDir}" && cat > "${configPath}" << 'EOF'\n${configContent}\nEOF`];
        ...
        if (_lastGeneratedAlttabContent !== alttabContent) {
            ...
            writeAlttabProcess.command = ["sh", "-c", `mkdir -p "${niriAdvsDir}" && cat > "${alttabPath}" << 'EOF'\n${alttabContent}\nEOF`];
            writeAlttabProcess.running = true;
        }

        for (const name of ["outputs", "binds", "cursor", "windowrules", "colors", "alttab", "layout", "input"]) {
            const path = niriAdvsDir + "/" + name + ".kdl";
            Proc.runCommand("niri-ensure-" + name, ["sh", "-c", `mkdir -p "${niriAdvsDir}" && [ ! -f "${path}" ] && touch "${path}" || true`], (output, exitCode) => {
                if (exitCode !== 0)
    ...
    function generateNiriBlurrule() {
        ...
        const configDir = Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation));
        const niriAdvsDir = configDir + "/niri/advs";
        const blurrulePath = niriAdvsDir + "/wpblur.kdl";
        const sourceBlurrulePath = Paths.strip(Qt.resolvedUrl("niri-wpblur.kdl"));

        writeBlurruleProcess.blurrulePath = blurrulePath;
        writeBlurruleProcess.command = ["sh", "-c", `mkdir -p "${niriAdvsDir}" && cp --no-preserve=mode "${sourceBlurrulePath}" "${blurrulePath}"`];
    ...
    function generateNiriCursorConfig() {
        ...
        const configDir = Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation));
        const niriAdvsDir = configDir + "/niri/advs";
        const cursorPath = niriAdvsDir + "/cursor.kdl";
        ...
        if (!settings) {
            ...
            writeCursorProcess.command = ["sh", "-c", `mkdir -p "${niriAdvsDir}" && : > "${cursorPath}"`];
...
`;
        ...
        let cursorContent = advsWarning + `cursor {\n`;
        ...
        writeCursorProcess.command = ["sh", "-c", `mkdir -p "${niriAdvsDir}" && printf '%s' '${escapedCursorContent}' > "${cursorPath}"`];
        writeCursorProcess.running = true;
    }
    ...
    function buildOutputsConfig(outputsData, niriSettings) {
        ...
        let kdlContent = `// Auto-generated by ADVS - do not edit manually\n\n`;
    ...
    function generateOutputsConfig(outputsData, settingsOrCallback, maybeCallback) {
        ...
        const configDir = Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation));
        const niriAdvsDir = configDir + "/niri/advs";
        const outputsPath = niriAdvsDir + "/outputs.kdl";

        Proc.runCommand("niri-write-outputs", ["sh", "-c", `mkdir -p "${niriAdvsDir}" && cat > "${outputsPath}" << 'EOF'\n${kdlContent}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
...
`;
        ...
        let inputContent = advsWarning + "input {\n";
        ...
        const configDir = Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation));
        const niriAdvsDir = configDir + "/niri/advs";
        const inputPath = niriAdvsDir + "/input.kdl";

        writeInputProcess.inputContent = inputContent;
        writeInputProcess.inputPath = inputPath;
        writeInputProcess.command = ["sh", "-c", `mkdir -p "${niriAdvsDir}" && cat > "${inputPath}" << 'EOF'\n${inputContent}\nEOF`];
