...
Singleton {
    id: root
    readonly property var log: Log.scoped("ADVSService")

    readonly property bool advsAvailable: isConnected
    ...
    property string updateCommand: "advs update"
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (helper.includes("paru")) {
                    checkAdvsPackage.helper = "paru";
                    checkAdvsPackage.running = true;
                } else if (helper.includes("yay")) {
                    checkAdvsPackage.helper = "yay";
                    checkAdvsPackage.running = true;
                } else {
                    updateCommand = "advs update";
    ...
    Process {
        ...
        command: ["sh", "-c", "pacman -Qi ariadnev-shell-git 2>/dev/null || pacman -Qi ariadnev-shell-bin 2>/dev/null"]
        running: false

        stdout: StdioCollector {
            onStreamFinished: {
                if (text.includes("ariadnev-shell-git")) {
                    updateCommand = checkAdvsPackage.helper + " -S ariadnev-shell-git";
                } else if (text.includes("ariadnev-shell-bin")) {
                    updateCommand = checkAdvsPackage.helper + " -S ariadnev-shell-bin";
                } else {
                    updateCommand = "advs update";
                }
                checkingUpdateCommand = false;
    ...
    function handleSubscriptionEvent(response) {
        if (response.error) {
            if (response.error.includes("unknown method") && response.error.includes("subscribe")) {
                if (!shownOutdatedError) {
                    ...
                    ToastService.showError(I18n.tr("ADVS out of date"), I18n.tr("To update, run the following command:"), updateCommand);
        ...
        if (service === "server") {
            ...
            if (apiVersion < expectedApiVersion) {
                ToastService.showError(I18n.tr("ADVS server is outdated (API v%1, expected v%2)").arg(apiVersion).arg(expectedApiVersion));
            }
    ...
    function sendRequest(method, params, callback) {
        if (!isConnected) {
            log.warn("ADVSService.sendRequest: Not connected, method:", method);
            if (callback) {
                callback({
                    "error": "not connected to ADVS socket"
                });
            }
            return;
        }
        ...
        if (method.startsWith("clipboard")) {
            ...
            log.debug("ADVSService.sendRequest: Sending request id=" + id + " method=" + method);
        }
    ...
    readonly property string dbusClientId: "advs-qml-" + Date.now() + "-" + Math.floor(Math.random() * 0xffffffff)
