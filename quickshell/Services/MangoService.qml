...
Singleton {
    ...
    readonly property string mangoAdvsDir: configDir + "/mango/advs"
    readonly property string bindsPath: mangoAdvsDir + "/binds.conf"
    readonly property string colorsPath: mangoAdvsDir + "/colors.conf"
    readonly property string outputsPath: mangoAdvsDir + "/outputs.conf"
    readonly property string layoutPath: mangoAdvsDir + "/layout.conf"
    readonly property string cursorPath: mangoAdvsDir + "/cursor.conf"
    readonly property string windowRulesPath: mangoAdvsDir + "/windowrules.conf"
    ...
    function generateOutputsConfig(outputsData, callback, skipReload) {
        ...
        suppressWatchedConfigReloads(1500);
        Proc.runCommand("mango-write-outputs", ["sh", "-c", `mkdir -p "${mangoAdvsDir}" && cat > "${outputsPath}" << 'EOF'\n${content}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
...
`;
        ...
        suppressWatchedConfigReloads(1500);
        Proc.runCommand("mango-write-layout", ["sh", "-c", `mkdir -p "${mangoAdvsDir}" && cat > "${layoutPath}" << 'EOF'\n${content}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
    ...
    function generateCursorConfig() {
        ...
        if (!settings) {
            suppressWatchedConfigReloads(1500);
            Proc.runCommand("mango-write-cursor", ["sh", "-c", `mkdir -p "${mangoAdvsDir}" && : > "${cursorPath}"`], (output, exitCode) => {
                if (exitCode !== 0)
...
cursor_size=${size}`;
        ...
        suppressWatchedConfigReloads(1500);
        Proc.runCommand("mango-write-cursor", ["sh", "-c", `mkdir -p "${mangoAdvsDir}" && cat > "${cursorPath}" << 'EOF'\n${content}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
                log.warn("Failed to write cursor config:", output);
                return;
            }
