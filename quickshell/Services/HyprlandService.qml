...
Singleton {
    ...
    readonly property string hyprAdvsDir: configDir + "/hypr/advs"
    readonly property string outputsPath: hyprAdvsDir + "/outputs.lua"
    readonly property string layoutPath: hyprAdvsDir + "/layout.lua"
    readonly property string cursorPath: hyprAdvsDir + "/cursor.lua"
    readonly property string windowrulesPath: hyprAdvsDir + "/windowrules.lua"
    ...
    // advs/layout.lua is the source of truth for xray; parsed once before the first regeneration
    ...
    function ensureWindowrulesConfig() {
        if (!canWriteLuaConfig("windowrules"))
            return;
        Proc.runCommand("hypr-ensure-windowrules", ["sh", "-c", `mkdir -p "${hyprAdvsDir}" && [ ! -f "${windowrulesPath}" ] && touch "${windowrulesPath}" || true`], (output, exitCode) => {
            if (exitCode !== 0)
    ...
    function refreshLuaConfigStatus() {
        ...
        Proc.runCommand("hypr-lua-config-status", [Proc.advsBin, "config", "resolve-include", "hyprland", "outputs.lua"], (output, exitCode) => {
            luaConfigStatusLoading = false;
    ...
    function generateOutputsConfig(outputsData, hyprlandSettings, callback, skipReload) {
        ...
        let lines = ["-- Auto-generated by ADVS — do not edit manually", ""];
        ...
        Proc.runCommand("hypr-write-outputs", ["sh", "-c", `mkdir -p "${hyprAdvsDir}" && cat > "${outputsPath}" << 'EOF'\n${content}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
    ...
    function loadLayoutXrayState() {
        ...
        Proc.runCommand("hypr-read-layout-xray", ["cat", configDir + "/hypr/advs/layout.lua"], (output, exitCode) => {
            _layoutXrayLoading = false;
            if (!_layoutXrayLoaded) {
                const content = exitCode === 0 ? output : "";
                layoutXrayEnabled = content.includes('"^advs:.*$"');
    ...
    function doGenerateLayoutConfig() {
        ...
        let content = `-- Auto-generated by ADVS — do not edit manually
...
`;
        ...
        Proc.runCommand("hypr-write-layout", ["sh", "-c", `mkdir -p "${hyprAdvsDir}" && cat > "${layoutPath}" << 'EOF'\n${content}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
    ...
    function generateCursorConfig() {
        ...
        if (!settings) {
            Proc.runCommand("hypr-write-cursor", ["sh", "-c", `mkdir -p "${hyprAdvsDir}" && printf '%s\\n' "-- Auto-generated by ADVS — do not edit manually" "" > "${cursorPath}"`], (output, exitCode) => {
                if (exitCode !== 0)
        ...
        Proc.runCommand("hypr-write-cursor", ["sh", "-c", `mkdir -p "${hyprAdvsDir}" && cat > "${cursorPath}" << 'EOF'\n${content}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
                log.warn("Failed to write cursor config:", output);
                return;
            }
