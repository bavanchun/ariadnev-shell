...
Item {
    ...
    Connections {
        ...
        function onSelectedBarIdChanged() {
            if (advBarTab.selectedBarId !== SettingsUiState.selectedBarId)
                advBarTab.selectedBarId = SettingsUiState.selectedBarId;
        }
    }
    ...
    readonly property bool islandShadowsSelectedBar: {
        ...
        Quickshell.screens;
        selectedBarId;
        if (!selectedBarConfig || advBarTab.selectedBarIsIsland)
            return false;
        const edge = SettingsData.positionToSide(selectedBarConfig.position ?? SettingsData.Position.Top);
        return Quickshell.screens.some(screen => SettingsData.barConfigCoversScreen(selectedBarConfig, screen) && SettingsData.advIslandOwnsEdge(screen, edge));
    }
    readonly property bool islandOwnsSelectedBarTop: advBarTab.selectedBarIsIsland || advBarTab.islandShadowsSelectedBar
    readonly property var positionChoices: {
        ...
        if (advBarTab.selectedBarIsIsland)
    ...
    function deleteBar(barId) {
        if (barId === "default")
            return;
        if (SettingsData.getBarKindConfigs().length <= 1 && barId !== SettingsData.advIslandBarId)
            return;
    ...
    function toggleBarEnabled(barId) {
        if (barId === "default" && barId !== SettingsData.advIslandBarId)
            return;
    ...
    AdvFlickable {
        ...
        Column {
            ...
            SettingsCard {
                tab: "appearance"
                iconName: "toolbar"
                title: I18n.tr("Adv Bar")
                settingKey: "barAppearance"
                visible: advBarTab.appearanceOnly

                SettingsButtonGroupRow {
                    text: I18n.tr("Editing changes on %1").arg(advBarTab.selectedBarName)
                    model: SettingsData.barConfigs.map((config, index) => config.name || I18n.tr("Bar %1").arg(index + 1))
                    currentIndex: {
                        const index = SettingsData.barConfigs.findIndex(config => config.id === advBarTab.selectedBarId);
                        return Math.max(0, index);
                    }
                    onSelectionChanged: (index, selected) => {
                        if (!selected || index < 0 || index >= SettingsData.barConfigs.length)
                            return;
                        advBarTab.selectedBarId = SettingsData.barConfigs[index].id;
                    }
                }
            }
            ...
            SettingsCard {
                ...
                RowLayout {
                    ...
                    AdvButton {
                        ...
                        onClicked: advBarTab.createNewBar()
                    }
                }
                ...
                Column {
                    ...
                    Repeater {
                        ...
                        Rectangle {
                            ...
                            color: advBarTab.selectedBarId === modelData.id ? Theme.withAlpha(Theme.primary, 0.15) : Theme.surfaceVariant
                            border.width: advBarTab.selectedBarId === modelData.id ? 2 : 0
                            ...
                            Row {
                                ...
                                Column {
                                    ...
                                    Row {
                                        ...
                                        StyledText {
                                            ...
                                            color: Theme.surfaceVariantText
                                            horizontalAlignment: Text.AlignLeft
                                            visible: SettingsData.advIslandBarId === barCard.modelData.id
                                        }
                                ...
                                AdvActionButton {
                                    ...
                                    enabled: SettingsData.getBarKindConfigs().length > 1 || SettingsData.advIslandBarId === barCard.modelData.id
                                    anchors.verticalCenter: parent.verticalCenter
                                    onClicked: advBarTab.deleteBar(barCard.modelData.id)
                                }
                            }
                            ...
                            MouseArea {
                                ...
                                onClicked: advBarTab.selectedBarId = barCard.modelData.id
                            }
            ...
            SettingsCard {
                ...
                title: advBarTab.selectedBarIsIsland ? I18n.tr("Enable Island") : I18n.tr("Enable Bar")
                visible: !advBarTab.appearanceOnly && (selectedBarId !== "default" || advBarTab.selectedBarIsIsland)

                SettingsToggleRow {
                    text: advBarTab.selectedBarIsIsland ? I18n.tr("Toggle the Island on this instance's displays") : I18n.tr("Toggle visibility of this bar configuration")
                    checked: {
                        selectedBarId;
                        return selectedBarConfig?.enabled ?? false;
                    }
                    onToggled: toggled => advBarTab.toggleBarEnabled(selectedBarId)
                }
            }
            ...
            SettingsCard {
                ...
                visible: !advBarTab.appearanceOnly && selectedBarConfig?.enabled

                SettingsControlledBy {
                    target: "island"
                    visible: advBarTab.selectedBarIsIsland
                    parentModal: advBarTab.parentModal
                ...
                SettingsControlledBy {
                    target: "island"
                    visible: advBarTab.islandShadowsSelectedBar
                    parentModal: advBarTab.parentModal
                    settingLabel: I18n.tr("%1 position").arg(advBarTab.positionLabel(selectedBarConfig?.position ?? SettingsData.Position.Top))
                ...
                Item {
                    ...
                    AdvButtonGroup {
                        ...
                        anchors.horizontalCenter: parent.horizontalCenter
                        model: advBarTab.positionChoices.map(pos => advBarTab.positionLabel(pos))
                        currentIndex: {
                            selectedBarId;
                            const config = SettingsData.getBarConfig(selectedBarId);
                            return advBarTab.positionChoices.indexOf(config?.position ?? SettingsData.Position.Top);
                        }
                        onSelectionChanged: (index, selected) => {
                            if (!selected || index < 0 || index >= advBarTab.positionChoices.length)
                                return;
                            SettingsData.updateBarConfig(selectedBarId, {
                                position: advBarTab.positionChoices[index]
                            });
                            notifyHorizontalBarChange();
                        }
                    }
            ...
            SettingsCard {
                ...
                StyledText {
                    width: parent.width
                    text: advBarTab.selectedBarIsIsland ? I18n.tr("Configure which displays show Adv Island as \"%1\"").replace("%1", selectedBarConfig?.name || "this instance") : I18n.tr("Configure which displays show \"%1\"").replace("%1", selectedBarConfig?.name || "this bar")
                    font.pixelSize: Theme.fontSizeSmall
                    color: Theme.surfaceVariantText
                    wrapMode: Text.WordWrap
                    horizontalAlignment: Text.AlignLeft
                ...
                Column {
                    ...
                    SettingsToggleRow {
                        ...
                        onToggled: checked => {
                            if (checked) {
                                advBarTab.setBarScreenPreferences(selectedBarId, ["all"]);
                            } else {
                                advBarTab.setBarScreenPreferences(selectedBarId, []);
                            }
                        }
                    }
                    ...
                    SettingsToggleRow {
                        ...
                        onToggled: checked => advBarTab.setBarShowOnLastDisplay(selectedBarId, checked)
                    }
                        ...
                        Repeater {
                            ...
                            delegate: SettingsToggleRow {
                                ...
                                onToggled: checked => {
                                    ...
                                    advBarTab.setBarScreenPreferences(selectedBarId, newPrefs);
                                }
                            }
                        }
                    }
            ...
            SettingsCard {
                ...
                SettingsControlledBy {
                    target: "island"
                    visible: advBarTab.islandOwnsSelectedBarTop
                    parentModal: advBarTab.parentModal
                ...
                Column {
                    width: parent.width
                    spacing: Theme.spacingS
                    visible: (selectedBarConfig?.autoHide ?? false) && !advBarTab.islandOwnsSelectedBarTop
                    ...
                    SettingsToggleRow {
                        ...
                        width: parent.width - parent.leftPadding
                        text: I18n.tr("Strict auto-hide", "Adv bar setting: hide the bar when the pointer leaves even if a menu or bar popover is still open")
                ...
                Rectangle {
                    ...
                    color: Theme.outline
                    opacity: 0.15
                    visible: CompositorService.isNiri && !advBarTab.islandOwnsSelectedBarTop
                }
            ...
            SettingsCard {
                ...
                SettingsSliderRow {
                    ...
                    visible: !SettingsData.frameEnabled && !advBarTab.islandOwnsSelectedBarTop
            ...
            SettingsCard {
                ...
                visible: advBarTab.appearanceOnly && (selectedBarConfig?.enabled ?? false)
                ...
                SettingsSliderRow {
                    ...
                    minimum: 0
                    maximum: 48
                    defaultValue: Math.round(advBarTab.insetPadAutoUI)
                    value: advBarTab.insetPadDisplayValue
                    onSliderDragFinished: finalValue => {
            ...
            SettingsSliderCard {
                ...
                description: I18n.tr("Scale AdvBar font sizes independently")
                visible: advBarTab.appearanceOnly && selectedBarConfig?.enabled
                minimum: 50
                maximum: 200
            ...
            SettingsSliderCard {
                ...
                description: I18n.tr("Scale AdvBar icon sizes independently")
                visible: advBarTab.appearanceOnly && selectedBarConfig?.enabled
                minimum: 50
                maximum: 200
            ...
            SettingsCard {
                ...
                SettingsToggleRow {
                    ...
                    visible: (selectedBarConfig?.gothCornersEnabled ?? false) && !advBarTab.islandOwnsSelectedBarTop
                    onToggled: checked => SettingsData.updateBarConfig(selectedBarId, {
                ...
                Column {
                    width: parent.width
                    spacing: Theme.spacingS
                    visible: (selectedBarConfig?.gothCornersEnabled ?? false) && (selectedBarConfig?.gothCornerRadiusOverride ?? false) && !advBarTab.islandOwnsSelectedBarTop
                    leftPadding: Theme.spacingM
            ...
            SettingsToggleCard {
                ...
                visible: !advBarTab.appearanceOnly && selectedBarConfig?.enabled && (CompositorService.isNiri || CompositorService.isHyprland || CompositorService.isMango)
            ...
            SettingsToggleCard {
                ...
                visible: advBarTab.appearanceOnly && selectedBarConfig?.enabled && !advBarTab.connectedFrameModeActive && !advBarTab.islandOwnsSelectedBarTop
            ...
            SettingsControlledBy {
                visible: advBarTab.appearanceOnly && advBarTab.connectedFrameModeActive
                parentModal: advBarTab.parentModal
            ...
            SettingsControlledBy {
                target: "island"
                visible: advBarTab.appearanceOnly && !advBarTab.connectedFrameModeActive && advBarTab.islandOwnsSelectedBarTop
                parentModal: advBarTab.parentModal
            ...
            SettingsCard {
                ...
                collapsible: true
                expanded: false
                visible: advBarTab.appearanceOnly && (selectedBarConfig?.enabled ?? false) && !advBarTab.connectedFrameModeActive && !advBarTab.islandOwnsSelectedBarTop
