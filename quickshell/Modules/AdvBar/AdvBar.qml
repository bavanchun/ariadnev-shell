...
Item {
    ...
    Component.onCompleted: BarWidgetService.registerAdvBarItem(root.barConfig?.id, root)
    Component.onDestruction: BarWidgetService.unregisterAdvBarItem(root.barConfig?.id, root)
    ...
    Variants {
        ...
        model: {
            ...
            return base.filter(screen => {
                ...
                return !SettingsData.advIslandOwnsEdge(screen, edge);
            });
        }

        delegate: AdvBarWindow {
            rootWindow: root
