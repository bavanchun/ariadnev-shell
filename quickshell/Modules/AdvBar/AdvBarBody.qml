...
Item {
    id: barWindow
    readonly property var log: Log.scoped("AdvBarBody")
    ...
    function triggerDashTab(tabId, position) {
        const loader = PopoutService.advDashPopoutLoader;
        if (!loader)
            return false;
        loader.active = true;
        if (!loader.item) {
    ...
    Connections {
        ...
        function onPluginLoaded(pluginId) {
            log.info("AdvBar: Plugin loaded:", pluginId);
            SettingsData.widgetDataChanged();
        }
        function onPluginUnloaded(pluginId) {
            log.info("AdvBar: Plugin unloaded:", pluginId);
            SettingsData.widgetDataChanged();
        }
    }
