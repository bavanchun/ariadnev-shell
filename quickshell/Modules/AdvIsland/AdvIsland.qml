...
Item {
    ...
    function ipcOpen(activity, screen) {
        ...
        if (!host)
            return "ADV_ISLAND_UNAVAILABLE";
        const requested = root.activityName(activity);
        if (!root.openActivityOn(host, requested, ""))
            return `ADV_ISLAND_ACTIVITY_UNAVAILABLE: ${requested}`;
        return `ADV_ISLAND_OPEN: ${requested}\t${host.screen?.name ?? ""}`;
    }
    ...
    function ipcToggle(activity, screen) {
        ...
        if (host.islandController.expanded && !host.islandController.notificationActive) {
            host.islandController.requestCollapse();
            return `ADV_ISLAND_CLOSED: ${host.screen?.name ?? ""}`;
        }
    ...
    function ipcShow(activity, screen) {
        ...
        if (!host.islandController.requestActivity(requested, false, false))
            return `ADV_ISLAND_ACTIVITY_UNAVAILABLE: ${requested}`;
        return `ADV_ISLAND_SHOW: ${requested}\t${host.screen?.name ?? ""}`;
    }
    ...
    function ipcCycle(screen) {
        ...
        return `ADV_ISLAND_ACTIVITY: ${host.islandController.activeActivity}\t${host.screen?.name ?? ""}`;
    }
    ...
    Component.onCompleted: PopoutService.advIslandRouter = root
    Component.onDestruction: {
        if (PopoutService.advIslandRouter === root)
            PopoutService.advIslandRouter = null;
    }
