...
import qs.Common
import qs.Modals.AdvLauncherV2 as AdvLauncher
...
PanelWindow {
    ...
    readonly property int reserveHeight: Math.max(24, Math.min(128, SettingsData.advIslandReserveHeight))
    readonly property int compactHeight: Math.max(24, Math.min(72, SettingsData.advIslandCompactHeight))
    readonly property bool floating: SettingsData.advIslandFloating
    readonly property bool usesOverlayLayer: LayerShell.envUsesOverlay("ADVS_ADVISLAND_LAYER", SettingsData.advIslandUseOverlayLayer)
    readonly property var islandLayer: LayerShell.fromEnv("ADVS_ADVISLAND_LAYER", usesOverlayLayer ? WlrLayer.Overlay : WlrLayer.Top)
    readonly property bool bottomEdge: SettingsData.advIslandEdge === "bottom"
    ...
    readonly property int outerGap: Math.max(0, Math.min(48, SettingsData.advIslandOuterGap))
    ...
    WlrLayershell.namespace: "advs:advisland"
    ...
    IslandController {
        ...
        interactionMode: SettingsData.advIslandInteractionMode === "click" ? "click" : "hybrid"
        inputSuspended: PopoutManager.screenshotActive
        horizontalOffset: Math.max(-root.maximumHorizontalOffset, Math.min(root.maximumHorizontalOffset, SettingsData.advIslandHorizontalOffset))
        ...
        notificationExpandAllowed: SettingsData.advIslandNotificationExpand
        unreadNotificationCount: SettingsData.advIslandNotificationBadgeClearOnOpen ? NotificationService.unreadCount : NotificationService.notifications.length
        hoverOpenDelay: Math.max(0, Math.min(1000, SettingsData.advIslandHoverOpenDelay))
        hoverCloseDelay: Math.max(0, Math.min(1000, SettingsData.advIslandHoverCloseDelay))
    }

    AdvLauncher.Controller {
    ...
    AdvIslandSurface {
        ...
        reducedMotion: SettingsData.advIslandReducedMotion || SettingsData.reduceMotion || SettingsData.animationSpeed === SettingsData.AnimationSpeed.None
        springStiffness: Math.max(100, Math.min(1200, SettingsData.advIslandSpringStiffness))
        springDamping: Math.max(10, Math.min(100, SettingsData.advIslandSpringDamping))
        springMass: Math.max(0.25, Math.min(3, SettingsData.advIslandSpringMass))
        palette: SettingsData.advIslandPalette
        highContrast: SettingsData.advIslandHighContrast
    ...
    PanelWindow {
        ...
        WlrLayershell.namespace: "advs:advisland:dismiss"
