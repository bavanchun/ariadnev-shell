...
Item {
    ...
    readonly property bool edgeAligned: SettingsData.advIslandSatellitePosition === "edges"
    ...
    readonly property bool backgroundEnabled: SettingsData.advIslandSatelliteBackground
    ...
    readonly property bool gothCorners: SettingsData.advIslandSatelliteGothCorners
    readonly property real chromeOpacity: Math.max(0, Math.min(1, SettingsData.advIslandSatelliteTransparency))
    readonly property real sweepRadius: Math.max(4, Math.min(64, SettingsData.advIslandSatelliteSwoopRadius))
    readonly property color chromeBase: {
        if (SettingsData.advIslandHighContrast)
            return Theme.surfaceContainerHighest;
        switch (SettingsData.advIslandPalette) {
        case "bright":
    ...
    AxisContext {
        ...
        edge: SettingsData.advIslandEdge
    }
    ...
    Item {
        ...
        x: (root.spanEdges ? 0 : (root.motionRunning ? Math.min(root.islandStartX, root.islandTargetX) - SettingsData.advIslandSatelliteGap - leftInput.width : leftInput.x)) - (root.spanEdges ? 0 : root.chromeInset)
    ...
    Item {
        ...
        x: (root.spanEdges ? edgeX : (root.motionRunning ? Math.min(root.islandStartRight, root.islandTargetRight) + SettingsData.advIslandSatelliteGap : rightInput.x)) - (root.spanEdges ? 0 : root.chromeInset)
    ...
    Item {
        ...
        x: root.edgeAligned ? root.edgeInset : Math.round(root.islandSurface.currentVisualX - SettingsData.advIslandSatelliteGap - width)
        y: root.rowY
    ...
    Item {
        ...
        x: root.edgeAligned ? root.width - root.edgeInset - width : Math.round(root.islandSurface.currentVisualX + root.islandSurface.currentVisualWidth + SettingsData.advIslandSatelliteGap)
        y: root.rowY
