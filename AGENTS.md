...
ADVS is an open-source, MIT-licensed desktop shell for Wayland compositors on linux. Like KWin, gnome-shell, or cosmic-shell - but designed to work with many different compositors.
...
1. cmd/advs - The parent process for quickshell (shell runner), and general purpose tools and utilities (such as screenshot, clipboard). It also runs the unix socket server the UI uses for functionality quickshell doesn't provide.
2. cmd/advinstall - An entirely separate app, a TUI for quickly installing ADVS

### 2- quickshell:

A quickshell application, the entirety of the UI for ADVS. It uses quickshells own services supplemented by the core unix socket server for additional functionality.
...
- Modules/ - the visible shell: AdvBar, ControlCenter, Dock, Lock, Notifications, OSD, and so on
- Modals/ - large standalone surfaces: Settings, AdvLauncherV2, Clipboard, Greeter
- Widgets/ - reusable Adv* components (AdvListView, AdvIcon, AdvToggle, ...)
...
- Resource usage is extremely important to ADVS. The shell runs 24/7 on every machine it's installed on - audit any change for idle CPU cost, extra processes, timers, and retained memory.
...
- Use the Adv* wrappers in Widgets/ (AdvListView, AdvFlickable, and so on) instead of raw ListView/Flickable/ScrollView.
