...
Create or update the ADVS-managed hotspot profile. Hotspot support is capability-gated: clients should require API v28+, `hotspotSupported: true`, and `hotspotAvailable: true` from network state before showing hotspot controls.
...
Configuration changes are rejected while the ADVS hotspot is active or activating; stop it before updating the profile.
...
Start the previously configured ADVS-managed hotspot profile. This action does not accept or require SSID/password parameters; call `network.hotspot.configure` first when changing hotspot settings.
...
Stop the active ADVS-managed hotspot connection. It must not stop arbitrary user-created hotspot profiles.
...
Retrieve the stored password of the ADVS-managed hotspot profile, for prefilling edit forms. Returns an empty string for an open hotspot. Network state exposes `hotspotSecured` so clients can tell an open hotspot apart from a secured one without fetching the secret.
...
- `savedWifiNetworks` (API v26+): Saved WiFi profiles exposed at SSID granularity. If a backend has multiple profiles for the same SSID, ADVS merges them into one SSID-level entry. Clients talking to older servers should derive saved visible networks from `wifiNetworks` entries where `saved` is true.
...
- `hotspotConfigured` (API v28+): Whether the ADVS-managed hotspot profile exists.
- `hotspotEnabled` (API v28+): Whether the ADVS-managed hotspot profile is currently active.
...
- `hotspotSSID` (API v28+): Configured ADVS hotspot SSID.
- `hotspotDevice` (API v28+): Optional configured Wi-Fi device for the ADVS hotspot.
