...
func LocateADVSConfig() (string, error) {
	var primaryPaths []string

	configHome, err := os.UserConfigDir()
	if err == nil && configHome != "" {
		primaryPaths = append(primaryPaths, filepath.Join(configHome, "quickshell", "advs"))
	}
	...
	for dir := range strings.SplitSeq(dataDirs, ":") {
		if dir != "" {
			primaryPaths = append(primaryPaths, filepath.Join(dir, "quickshell", "advs"))
		}
	}
	...
	return "", fmt.Errorf("could not find ADVS config (shell.qml) in any valid config path")
}
