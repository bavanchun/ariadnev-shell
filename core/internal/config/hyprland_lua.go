...
const (
	hyprlandStartupBegin = "-- ADVS_STARTUP_BEGIN"
	hyprlandStartupEnd   = "-- ADVS_STARTUP_END"
)
...
// top-level ~/.config/hypr/advs/*.conf files into .advs-backups/<timestamp>/ only
...
func CleanupStrayHyprlandConfFile(logFn func(format string, v ...any)) {
	...
	advsConfPaths, err := filepath.Glob(filepath.Join(configDir, "advs", "*.conf"))
	if err == nil {
		for _, p := range advsConfPaths {
