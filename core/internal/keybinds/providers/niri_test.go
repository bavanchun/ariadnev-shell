...
func TestNiriFormatRawAction(t *testing.T) {
	...
	tests := []struct {
		...
		{"spawn", []string{"advs", "ipc", "call"}, "spawn advs ipc call"},
		{"spawn", []string{"advs", "ipc", "call", "brightness", "increment", "5", ""}, `spawn advs ipc call brightness increment 5 ""`},
		{"spawn", []string{"advs", "ipc", "call", "dash", "toggle", ""}, `spawn advs ipc call dash toggle ""`},
...
`,
		},
		{
			...
			binds: map[string]*overrideBind{
				"Mod+Space": {
					Key:         "Mod+Space",
					Action:      `spawn "advs" "ipc" "call" "spotlight" "toggle"`,
					Description: "Application Launcher",
				},
			},
			expected: `binds {
    Mod+Space hotkey-overlay-title="Application Launcher" { spawn "advs" "ipc" "call" "spotlight" "toggle"; }
}
`,
		},
		{
...
`,
		},
		{
			...
			binds: map[string]*overrideBind{
				"XF86AudioMute": {
					Key:     "XF86AudioMute",
					Action:  `spawn "advs" "ipc" "call" "audio" "mute"`,
					Options: map[string]any{"allow-when-locked": true},
				},
			},
			expected: `binds {
    XF86AudioMute allow-when-locked=true { spawn "advs" "ipc" "call" "audio" "mute"; }
}
`,
		},
		{
...
func TestNiriEmptyArgsPreservation(t *testing.T) {
	...
	binds := map[string]*overrideBind{
		"XF86MonBrightnessUp": {
			...
			Action:      `spawn advs ipc call brightness increment 5 ""`,
			Description: "Brightness Up",
		},
		"XF86MonBrightnessDown": {
			Key:         "XF86MonBrightnessDown",
			Action:      `spawn advs ipc call brightness decrement 5 ""`,
			Description: "Brightness Down",
		},
		"Super+Alt+Page_Up": {
			Key:         "Super+Alt+Page_Up",
			Action:      `spawn advs ipc call dash toggle ""`,
	...
	if err := os.MkdirAll(advsDir, 0o755); err != nil {
		t.Fatalf("Failed to create advs directory: %v", err)
	}

	bindsFile := filepath.Join(advsDir, "binds.kdl")
...
func TestNiriProviderWithRealWorldConfig(t *testing.T) {
    ...
    XF86AudioRaiseVolume allow-when-locked=true {
        spawn "advs" "ipc" "call" "audio" "increment" "3";
    }
    XF86AudioLowerVolume allow-when-locked=true {
        spawn "advs" "ipc" "call" "audio" "decrement" "3";
    }
...
func TestNiriGenerateSpawnWithNumericArgs(t *testing.T) {
	...
	binds := map[string]*overrideBind{
		"XF86AudioLowerVolume": {
			...
			Action:  `spawn "advs" "ipc" "call" "audio" "decrement" "3"`,
			Options: map[string]any{"allow-when-locked": true},
		},
	}
	...
	content := provider.generateBindsContent(binds)
	expected := `binds {
    XF86AudioLowerVolume allow-when-locked=true { spawn "advs" "ipc" "call" "audio" "decrement" "3"; }
}
`
	if content != expected {
		t.Errorf("Content mismatch.\nGot:\n%s\nWant:\n%s", content, expected)
