...
func (e *pamTestEnv) writeSettings(t *testing.T, content string) {
	t.Helper()
	writeTestFile(t, filepath.Join(e.homeDir, ".config", "AriadnevShell", "settings.json"), content)
}
...
func TestSyncLockscreenPamConfigWithDeps(t *testing.T) {
	...
	t.Run("custom advshell file is skipped untouched", func(t *testing.T) {
		t.Parallel()

		env := newPamTestEnv(t)
		customContent := "#%PAM-1.0\nauth required pam_unix.so\n"
		env.writePamFile(t, "advshell", customContent)
		...
		if got := readFileString(t, env.advshellPath); got != customContent {
			t.Fatalf("custom advshell content changed\ngot:\n%s\nwant:\n%s", got, customContent)
		}
		if len(logs) == 0 || !strings.Contains(logs[0], "Custom /etc/pam.d/advshell found") {
			t.Fatalf("expected custom-file skip log, got %v", logs)
		}
	})

	t.Run("managed advshell file is rewritten from resolved login stack", func(t *testing.T) {
		t.Parallel()

		env := newPamTestEnv(t)
		env.writePamFile(t, "login", "#%PAM-1.0\nauth include system-auth\naccount include system-auth\n")
		env.writePamFile(t, "system-auth", "auth sufficient pam_unix.so try_first_pass nullok\nauth sufficient pam_u2f.so cue\naccount required pam_access.so\n")
		env.writePamFile(t, "advshell", "#%PAM-1.0\n"+LockscreenPamManagedBlockStart+"\nauth required pam_env.so\n"+LockscreenPamManagedBlockEnd+"\n")
		...
		output := readFileString(t, env.advshellPath)
		for _, want := range []string{
			...
			if !strings.Contains(output, want) {
				t.Errorf("missing expected string %q in rewritten advshell:\n%s", want, output)
			}
		}
		if strings.Contains(output, "pam_u2f") {
			t.Errorf("rewritten advshell still contains pam_u2f:\n%s", output)
		}
		if len(logs) == 0 || !strings.Contains(logs[len(logs)-1], "Created or updated /etc/pam.d/advshell") {
	...
	t.Run("NixOS remains informational and does not write advshell", func(t *testing.T) {
		t.Parallel()
		...
		if _, err := os.Stat(env.advshellPath); !os.IsNotExist(err) {
			t.Fatalf("expected no advshell file to be written on NixOS path, stat err = %v", err)
		}
	})
}
...
func TestSyncLockscreenU2FPamConfigWithDeps(t *testing.T) {
	...
	t.Run("enabled creates managed file", func(t *testing.T) {
		...
		got := readFileString(t, env.advshellU2fPath)
		if got != buildManagedLockscreenU2FPamContent() {
			t.Fatalf("unexpected managed advshell-u2f content:\n%s", got)
		}
		if len(logs) == 0 || !strings.Contains(logs[len(logs)-1], "Created or updated /etc/pam.d/advshell-u2f") {
	...
	t.Run("enabled rewrites existing managed file", func(t *testing.T) {
		...
		env := newPamTestEnv(t)
		env.writePamFile(t, "advshell-u2f", "#%PAM-1.0\n"+LockscreenU2FPamManagedBlockStart+"\nauth required pam_u2f.so old\n"+LockscreenU2FPamManagedBlockEnd+"\n")

		if err := syncLockscreenU2FPamConfigWithDeps(func(string) {}, "", true, env.deps(false)); err != nil {
			t.Fatalf("syncLockscreenU2FPamConfigWithDeps returned error: %v", err)
		}
		if got := readFileString(t, env.advshellU2fPath); got != buildManagedLockscreenU2FPamContent() {
			t.Fatalf("managed advshell-u2f was not rewritten:\n%s", got)
		}
	})

	t.Run("disabled removes ADVS-managed file", func(t *testing.T) {
		t.Parallel()

		env := newPamTestEnv(t)
		env.writePamFile(t, "advshell-u2f", buildManagedLockscreenU2FPamContent())
		...
		if _, err := os.Stat(env.advshellU2fPath); !os.IsNotExist(err) {
			t.Fatalf("expected managed advshell-u2f to be removed, stat err = %v", err)
		}
		if len(logs) == 0 || !strings.Contains(logs[len(logs)-1], "Removed ADVS-managed /etc/pam.d/advshell-u2f") {
	...
	t.Run("disabled preserves custom file", func(t *testing.T) {
		...
		env.writePamFile(t, "advshell-u2f", customContent)
		...
		if got := readFileString(t, env.advshellU2fPath); got != customContent {
			t.Fatalf("custom advshell-u2f content changed\ngot:\n%s\nwant:\n%s", got, customContent)
		}
		if len(logs) == 0 || !strings.Contains(logs[0], "Custom /etc/pam.d/advshell-u2f found") {
...
func TestValidateLockscreenU2fPam(t *testing.T) {
	...
	t.Run("accepts a dedicated U2F stack with custom options", func(t *testing.T) {
		...
		result := validateLockscreenU2fPam("advshell-u2f", "", env.validateDeps())
		if !result.Valid {
...
func TestSyncAuthConfigWithDeps(t *testing.T) {
	...
	t.Run("creates lockscreen targets", func(t *testing.T) {
		...
		if _, err := os.Stat(env.advshellPath); err != nil {
			t.Fatalf("expected advshell to be created: %v", err)
		}
		if got := readFileString(t, env.advshellU2fPath); got != buildManagedLockscreenU2FPamContent() {
			t.Fatalf("unexpected advshell-u2f content:\n%s", got)
		}
	})

	t.Run("removes advshell-u2f when disabled", func(t *testing.T) {
		t.Parallel()
		...
		if _, err := os.Stat(env.advshellU2fPath); !os.IsNotExist(err) {
			t.Fatalf("expected advshell-u2f to be removed, stat err = %v", err)
		}
	})
}
