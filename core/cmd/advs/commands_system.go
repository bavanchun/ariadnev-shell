...
var systemCmd = &cobra.Command{
	...
	Long:  "System-level operations (updates, etc.). Runs against installed package managers directly; does not require the ADVS server.",
}
...
Examples:
  advs system update --check                  # list available updates
  advs system update                          # apply updates (interactive prompt)
  advs system update --noconfirm              # apply updates without prompting
  advs system update --dry                    # simulate without changing anything
  advs system update --no-flatpak --noconfirm # apply system updates only
  advs system update --interval 3600          # set the server poll interval to 1h`,
	Run: runSystemUpdate,
}
...
func init() {
	...
	systemUpdateCmd.Flags().IntVar(&sysUpdateIntervalS, "interval", -1, "Set the ADVS server poll interval in seconds and exit (requires running server)")
...
func runSystemUpdateSetInterval(seconds int) {
	...
	if err != nil {
		log.Fatalf("Failed: %v (is advs server running?)", err)
	}
	if resp.Error != "" {
