...
func checkHyprlandInclude(filename string) (IncludeResult, error) {
	...
	targetPath := filepath.Join(configDir, "advs", filename)
	...
	targetRel := filepath.ToSlash(filepath.Join("advs", filename))
...
func checkNiriInclude(filename string) (IncludeResult, error) {
	...
	processed := make(map[string]bool)
	result.Included = niriFindInclude(mainConfig, "advs/"+filename, processed)
	return result, nil
}
...
func checkMangoWCInclude(filename string) (IncludeResult, error) {
	...
	processed := make(map[string]bool)
	result.Included = mangowcFindInclude(mainConfig, "advs/"+filename, processed)
	return result, nil
}
