...
import (
	...
	"github.com/bavanchun/ariadnev-shell/core/internal/keybinds/providers"
...
var keybindsRemoveCmd = &cobra.Command{
	...
	Long:  "Remove a keybind. For Hyprland this writes a negative override to advs/binds-user.lua so the key stays unbound across ADVS updates. For other providers it deletes the entry from the managed file.",
...
var keybindsResetCmd = &cobra.Command{
	...
	Short: "Reset a keybind override to its ADVS default",
	Long:  "Drop the user override for the given key so the ADVS default re-applies. For providers without a separate default file (Niri, MangoWC) this is equivalent to remove.",
	Args:  cobra.ExactArgs(2),
