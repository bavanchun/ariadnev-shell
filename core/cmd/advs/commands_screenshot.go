...
import (
	...
	"github.com/bavanchun/ariadnev-shell/core/internal/notify"
	"github.com/bavanchun/ariadnev-shell/core/internal/screenshot"
...
Examples:
  advs screenshot                     # Region select, save file + clipboard
  advs screenshot full                # Full screen of focused output
  advs screenshot all                 # All screens combined
  advs screenshot output -o DP-1      # Specific output
  advs screenshot window              # Focused window (Hyprland)
  advs screenshot last                # Last region (pre-selected)
  advs screenshot --reset             # Reset last region pre-selection
  advs screenshot --no-clipboard      # Save file only
  advs screenshot --no-file           # Clipboard only
  advs screenshot --no-confirm        # Region capture on mouse release
  advs screenshot --cursor=on         # Include cursor
  advs screenshot -f jpg -q 85        # JPEG with quality 85
  advs screenshot --json              # Print capture metadata as JSON
  advs screenshot scroll              # Scroll capture, Enter finishes / Esc cancels
  advs screenshot scroll --interval 250`,
}
