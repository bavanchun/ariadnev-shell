...
Input:
  advs qr "https://example.com"     # encode an argument
  echo -n "data" | advs qr          # encode stdin
  advs qr -                         # encode stdin explicitly

Output (combine freely):
  advs qr "text" --clipboard        # copy PNG image to clipboard
  advs qr "text" --copy-text        # copy the source text to clipboard
  advs qr "text" -o code.png        # save a PNG file
  advs qr "text" > code.png         # PNG to stdout (piped)
  advs qr "text" --render           # force terminal render
...
WiFi:
  advs qr wifi MySSID -p secret     # build from an explicit password
  advs qr wifi MySSID               # pull the saved secret from the shell`,
...
credentials are fetched from the running ADVS shell (like the network panel).`,
