...
import (
	...
	"github.com/bavanchun/ariadnev-shell/core/internal/qsipc"
)
...
func printIPCHelp() {
	fmt.Println("Usage: advs ipc call <target> <function> [args...]")
	fmt.Println()
...
func logStartupFailure(exitCode int, uptime time.Duration, stderrTail string) {
	...
	if containsFontCrashSignature(stderrTail) {
		log.Errorf("ADVS startup failed due to a potential font/rendering crash. Try running 'fc-cache -fv' and restarting ADVS.")
	} else {
		log.Errorf("ADVS startup failed (exit code %d). Run 'advs doctor' for more diagnostics.", exitCode)
	}
}
