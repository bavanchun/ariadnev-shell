...
var openCmd = &cobra.Command{
	...
	Long: `Open a target (URL, file, or other resource) using the ADVS application picker.
...
Examples:
  advs open https://example.com                    # Open URL with browser picker
  advs open file.pdf                               # Open file (MIME auto-detected)
  advs open file.pdf --mime application/pdf        # Override MIME detection
  advs open document.odt --category Office         # Open with office applications`,
...
func runOpen(target string) {
	...
	case isScheme(target, "http://"), isScheme(target, "https://"), isScheme(target, "advs://"):
	...
	if detectedMimeType == "" && len(openCategories) == 0 && (isScheme(target, "http://") || isScheme(target, "https://") || isScheme(target, "advs://")) {
		method = "browser.open"
		params["url"] = target
	}
	...
	if err := sendServerRequestFireAndForget(req); err != nil {
		fmt.Println("ADVS is not running. Please start ADVS first.")
		os.Exit(1)
	}
