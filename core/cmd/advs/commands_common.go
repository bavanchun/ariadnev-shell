...
var ipcCmd = &cobra.Command{
	Use:   "ipc",
	Short: "Send IPC commands to running ADVS shell",
	Long: `Send IPC commands to the running ADVS shell.
...
var debugSrvCmd = &cobra.Command{
	...
	Long:  "Start the Unix socket debug server for ADVS",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsCmd = &cobra.Command{
	Use:   "plugins",
	Short: "Manage ADVS plugins",
	Long:  "Browse and manage ADVS plugins from the registry",
}

var pluginsBrowseCmd = &cobra.Command{
	Use:   "browse",
	Short: "Browse available plugins",
	Long:  "Browse available plugins from the ADVS plugin registry",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsListCmd = &cobra.Command{
	...
	Long:  "List all installed ADVS plugins",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsInstallCmd = &cobra.Command{
	...
	Long:  "Install a ADVS plugin from the registry using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
...
var pluginsUninstallCmd = &cobra.Command{
	...
	Long:  "Uninstall a ADVS plugin using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
			return nil, cobra.ShellCompDirectiveNoFileComp
...
var pluginsUpdateCmd = &cobra.Command{
	...
	Long:  "Update an installed ADVS plugin using its ID (e.g., 'myPlugin'). If --all or -a is specified, all installed plugins will be updated.",
...
// Git builds: advs (git) v0.6.2-XXXX
// Stable releases: advs v0.6.2
func formatVersion(version string) string {
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("advs (git) v%s-%s", matches[1], matches[2])
	}
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		...
		return fmt.Sprintf("advs (git) v%s-%s", baseVersion, matches[1])
	}

	// Stable release format: 0.6.2
	re = regexp.MustCompile(`^([\d.]+)$`)
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("advs v%s", matches[1])
	}

	return fmt.Sprintf("advs %s", version)
}
