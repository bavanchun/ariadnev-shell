...
var setupCmd = &cobra.Command{
	Use:               "setup",
	Short:             "Deploy ADVS configurations",
...
var setupBindsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSetupAdvsConfig("binds"); err != nil {
...
var setupLayoutCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSetupAdvsConfig("layout"); err != nil {
			log.Fatalf("Error: %v", err)
		}
	},
}
...
var setupColorsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSetupAdvsConfig("colors"); err != nil {
			log.Fatalf("Error: %v", err)
		}
	},
}
...
var setupAlttabCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSetupAdvsConfig("alttab"); err != nil {
			log.Fatalf("Error: %v", err)
		}
	},
}
...
var setupOutputsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSetupAdvsConfig("outputs"); err != nil {
			log.Fatalf("Error: %v", err)
		}
	},
}
...
var setupCursorCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSetupAdvsConfig("cursor"); err != nil {
			log.Fatalf("Error: %v", err)
		}
	},
}
...
var setupWindowrulesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := runSetupAdvsConfig("windowrules"); err != nil {
			log.Fatalf("Error: %v", err)
		}
	},
}

type advsConfigSpec struct {
	niriFile     string
	hyprFile     string
	mangoFile    string
...
var advsConfigSpecs = map[string]advsConfigSpec{
	"binds": {
		niriFile:  "binds.kdl",
		hyprFile:  "binds.lua",
		...
		hyprContent: func(t string) string {
			return strings.ReplaceAll(config.ADVSBindsLuaConfig, "{{TERMINAL_COMMAND}}", t)
		},
	...
	"layout": {
		...
		hyprContent:  func(_ string) string { return config.ADVSLayoutLuaConfig },
	...
	"colors": {
		...
		hyprContent:  func(_ string) string { return config.ADVSColorsLuaConfig },
	...
	"outputs": {
		...
		hyprContent:  func(_ string) string { return config.ADVSOutputsLuaConfig },
	...
	"cursor": {
		...
		niriContent:  func(_ string) string { return "" },
		hyprContent:  func(_ string) string { return config.ADVSCursorLuaConfig },
		mangoContent: func(_ string) string { return "" },
	},
	"windowrules": {
		...
		niriContent:  func(_ string) string { return "" },
		hyprContent:  func(_ string) string { return config.ADVSWindowRulesLuaConfig },
		mangoContent: func(_ string) string { return "" },
	},
}
...
func runSetupAdvsConfig(name string) error {
	spec, ok := advsConfigSpecs[name]
	if !ok {
	...
	switch compositor {
	case "niri":
		advsDir = filepath.Join(utils.XDGConfigHome(), "niri", "advs")
	case "hyprland":
		advsDir = filepath.Join(utils.XDGConfigHome(), "hypr", "advs")
	case "mango", "mangowc":
		advsDir = filepath.Join(utils.XDGConfigHome(), "mango", "advs")
	}
	...
	path := filepath.Join(advsDir, filename)
...
func runSetup() error {
	fmt.Println("=== ADVS Configuration Setup ===")
