...
var matugenCmd = &cobra.Command{
	...
	Long:  "Generate Material Design themes using matugen with adv16 color integration",
}
...
var matugenQtengineCmd = &cobra.Command{
	Use:   "qtengine",
	Short: "Sync qtengine's config with the current ADVS theme",
...
func init() {
	...
	for _, cmd := range []*cobra.Command{matugenGenerateCmd, matugenQueueCmd} {
		...
		cmd.Flags().String("shell-dir", "", "ADVS shell installation directory")
